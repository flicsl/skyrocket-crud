...
export class ServerNetworkCrud extends Component {
    constructor(schema, options = {}) {
        ...
        notifySchemaCreated[`SR_CRUD:NOTIFY_${schemaName}_CREATED`] = (event) => {
            const message = BufferUtils.concat(BufferUtils.toUint8(options.messageCreated), this._transformer.encode(event.player));
            options.sendToAll(message);
        };
        const notifySchemaUpdated = {};
        notifySchemaUpdated[`SR_CRUD:NOTIFY_${schemaName}_UPDATED`] = (event) => {
            const message = BufferUtils.concat(BufferUtils.toUint8(options.messageUpdated), this._transformer.encode(event.player));
            options.sendToAll(message);
        };
        const notifySchemaDeleted = {};
        notifySchemaDeleted[`SR_CRUD:NOTIFY_${schemaName}_DELETED`] = (event) => {
